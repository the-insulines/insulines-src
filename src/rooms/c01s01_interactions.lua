c01s01_objectInteractions = {
  ...
  floyd = {
    onClick = function ()
      if stateManager.state.c01s01.woke then
  ...
  josh_grabs_cellphone = {
    ...
    wakingUp = function ()
      -- Get Cellphone
      stateManager.state.c01s01.cellphonePicked = true
  ...
  cellphone = {
    ...
    onClick = function ( self )
      if not stateManager.state.c01s01.woke then
        if stateManager.state.c01s01.cellphoneClicks < 2 then
          if stateManager.state.c01s01.cellphoneClicks == 0 then
            self:firstClickReaction ()
          else
            self:secondClickReaction ()
          end
          stateManager.state.c01s01.cellphoneClicks = stateManager.state.c01s01.cellphoneClicks + 1
        else
          self:wakeUp ()
        end
      end
    ...
    wakeUp = function ( self )
      ...
      stateManager.state.c01s01.woke = true
  ...
  clothes_heap = {
    onClick = function ()
      ...
      if stateManager.state.c01s01.woke then
        if not stateManager.state.c01s01.dressed then
          game.currentScene:fadeOut ()
          performWithDelay (100, fadeIn, 1, game.currentScene)
          stateManager.state.c01s01.dressed = true
        end
      end
    end
  },
